...
public class Boss : LivingEntity
{
    ...
    public void AttackTrigger(Collider2D other)
    {
        ...
        {
            isAlreadyDamage = true;
            other.gameObject.GetComponent<BattleSwordman>().OnDamage(damage);
            if(isGroggyAttack)
            {
                isGroggyAttack = false;
                other.gameObject.GetComponent<BattleSwordman>().OnGroggy();
            }
        }
    }
    ...
    {
        base.OnDamage(dmg);
        BattleSoundManager.EnemyHitSound();
    ...
    {
        //Sound
        BattleSoundManager.BGMStop();
        BattleSoundManager.PlayGameWinSound();
    ...
    IEnumerator JumpEffect()
    {
        ...
        BattleSoundManager.VictoryUIPlay();
    }
}

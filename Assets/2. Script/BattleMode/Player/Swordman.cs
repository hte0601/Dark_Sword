...
{
    public class Swordman : LivingEntity
    {
        public static Swordman battleSwordman;
        ...
        void Start()
        {
            ...
            perfectDefenseTime = 0.3f - GameManager.DebuffPerfectDefenseTimeDown;
            staminaRegen = 30f - GameManager.DebuffStaminaRegenDown;
        ...
        private void UpdateXVector()
        {
            ...
            if (xVector != 0)
                SoundManager.PlayerFootSound();
            else
                SoundManager.playerSoundStop();
        }
        ...
        public override void OnDamage(float dmg)
        {
            if (isDefense)
            {
                if (defenseTiming < perfectDefenseTime)
                {
                    ...
                    ParticleManager.CreatePerfectDefenseParticle();
                    SoundManager.PlayerPerfectDefenseSound();
                }
                else if (defenseTiming < 1f) // 0.3second : 1second = 0dmg : 50dmg
                {
                    ...
                    SoundManager.PlayerDefenseSound();
                }
                else
                {
                    stamina -= 50f;
            ...
            ParticleManager.CreatePlayerHitParticle();
            base.OnDamage(dmg);
            hpBar.value = health / maxHealth * 100;
        }
        ...
        IEnumerator PierceAnimation()
        {
            ...
            isAlreadyDamage = false;
            SoundManager.PlayerPierceSound();
        ...
        IEnumerator SidecutAnimation()
        {
            ...
            stamina -= 15f;
            ParticleManager.CreateSidecutParticle();

            while (getNormalizedTime() < 0.35f)
                yield return null;

            isAlreadyDamage = false;
            SoundManager.PlayerSidecutSound();
        ...
        IEnumerator AvoidAnimation()
        {
            ...
            stamina -= 20f;
            SetSpeed(10f);
            ParticleManager.CreateAvoidParticle();
            SoundManager.PlayerAvoidSound();
            ...
            ParticleManager.StopAvoidParticle();
            xVector = 0f;
        ...
        IEnumerator GameOver()
        {
            ...
            GameManager.DebuffReset();
            SoundManager.GameOverUIPlay();
        }
        ...
        IEnumerator StaminaClock()
        {
            while (true)
            {
                if (isGroggy)
                {
                    ...
                    ParticleManager.CreatePlayerGroggyHitParticle();
        ...
        {
            if (GameManager.DeviceType == "PC")
            {
                return Input.GetAxisRaw("Horizontal");
            }
            else if (GameManager.DeviceType == "Mobile")
            {
        ...
        public static void AttackTrigger(Collider2D other)
        {
            if (battleSwordman.getState() == State.Attack && !battleSwordman.isAlreadyDamage)
            {
                ...
                ParticleManager.CreateEnemyHitParticle(battleSwordman.EnemyHitPoint.transform.position);
